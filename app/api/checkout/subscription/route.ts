...
import { getProductPriceId, STRIPE_PRODUCTS } from "@/lib/stripe/products";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!customerId) {
      ...
      // Update customer metadata to ensure user_id is set
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: {
        trial_period_days: undefined, // No additional trial for checkout (user already had 7-day trial)
        metadata: {
          user_id: user.id,
        },
      },
